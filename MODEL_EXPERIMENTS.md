...
合法真实 Trace 通过 `ExplorerStore.setTrace()` 原子切换：时间轴回到第一步、播放暂停、来源徽标变为“真实模型已就绪”，2D、3D、课程和自由探索继续读取同一份 Trace。再次打开真实模型面板可运行新参数，也可选择“恢复预置案例”；恢复后来源徽标和全部视图回到预置 Trace。

模型 Session 不会因关闭参数面板或恢复预置案例而立即释放，便于继续实验；页面卸载或 Worker 终止时统一释放。
