...
项目已完成 **M1：Token→Attention→Output 联动垂直切片**，M2 完整预置课程正在实施，WP-20～WP-21 已完成。
...
- 当前页面可以学习 Token、Embedding、LayerNorm、Q/K/V、Attention、Output 十项中文课程，在二维图中查看位置编码相加、归一化前后分布和三路投影，在 WebGL 场景中观察对应空间层，并使用可拖动时间轴定位；系统会按设备能力切换完整、简化或二维安全模式，保存本地进度，并在数据、子视图或 WebGL Context 失败时保持核心课程可用。
- 当前自动化基线为 131 条单元/组件测试和 12 条端到端场景。
...
| M2 | 完整预置 Transformer 课程 | 进行中（WP-20～WP-21 已完成） |
| M3 | 浏览器真实模型模式 | 待实施 |
