...
项目已完成 **M1：Token→Attention→Output 联动垂直切片**，M2 完整预置课程正在实施，WP-20～WP-24 已完成。
...
- 当前页面可以学习 Token、Embedding、LayerNorm、Q/K/V、Attention、Residual、MLP 与 Output 共十四项中文课程，在二维图中查看位置编码相加、归一化分布、三路投影、跨 Head 差异、两条残差旁路、`8→32→8` 前馈网络和完整词表概率，并通过 Temperature、Top-k、Top-p 与 Seed 实验可复现的采样结果；三维空间同步展示 CONCAT、Pre-Norm、MLP 与 Block 输出链路。系统会按设备能力切换完整、简化或二维安全模式，保存本地进度，并在数据、子视图或 WebGL Context 失败时保持核心课程可用。
- 当前自动化基线为 154 条单元/组件测试和 15 条端到端场景。
...
| M2 | 完整预置 Transformer 课程 | 进行中（WP-20～WP-24 已完成） |
| M3 | 浏览器真实模型模式 | 待实施 |
